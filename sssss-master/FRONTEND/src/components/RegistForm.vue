...
<!-- 
        ...
        <v-list-item class='d-flex-center' click="asdf">
          <v-list-item>
            <v-icon class='d-flex align-self-center' color="light-green darken-2">mdi-arrow-expand-all</v-icon>
            <v-list-item-title>매장 평수</v-list-item-title>
            <v-list-item-title>
              <v-text-field
                color='light-green lighten-4'
                v-model="Place.cafe.storearea"
                type="number"
                placeholder="120"
                :rules="[() => !!Place.placename || '매장 평수를 입력해주세요']"
                required
              ></v-text-field>
            </v-list-item-title>
          </v-list-item>
        ...
        <v-list-item>
          <v-list-item>
            <v-icon color="light-green darken-2">mdi-beer</v-icon>
            <v-list-item-title>주류 판매</v-list-item-title>
            <v-list-item-title>
              <v-radio-group 
                v-model="Place.cafe.alcoholonoff" row fluid >  
                <v-radio name='alcoholonoff' color="grey darken-1" label="판매 함" value="O" required></v-radio>
                <v-radio name='alcoholonoff' color="grey darken-1" label="판매 안함" value="X" required></v-radio>
              </v-radio-group>  
            </v-list-item-title>
          </v-list-item>
        </v-list-item>
        ...
        <v-list-item>
          <v-list-item>
            <v-icon color="light-green darken-2">mdi-cupcake</v-icon>
            <v-list-item-title>디저트 판매</v-list-item-title>
            <v-list-item-title>
              <v-radio-group v-model="Place.cafe.dessertonoff" row fluid>
                <v-radio name="dessertonoff" color="grey darken-1" label="판매 함" value="O" required></v-radio>
                <v-radio name="dessertonoff" color="grey darken-1" label="판매 안함" value="X" required></v-radio>
              </v-radio-group> 
            </v-list-item-title>
          </v-list-item>
        </v-list-item>
        ...
        <v-list-item>
          <v-list-item>
            <v-icon color="light-green darken-2">mdi-smoking</v-icon>
            <v-list-item-title>흡연실</v-list-item-title>
            <v-list-item-title>
              <v-radio-group v-model="Place.cafe.smokingroom" row fluid>
                <v-radio name="smokingroom" color="grey darken-1" label="있음" value="O" required></v-radio>
                <v-radio name="smokingroom" color="grey darken-1" label="없음" value="X" required></v-radio>
              </v-radio-group> 
            </v-list-item-title>
          </v-list-item>
        </v-list-item>
        ...
        <v-list-item>
          <v-list-item>
            <v-icon color="light-green darken-2">mdi-power-plug</v-icon>
            <v-list-item-title>콘센트 개수</v-list-item-title>
            <v-list-item-title>
              <v-text-field
                color='light-green lighten-4'
                v-model="Place.cafe.powersoket"
                type="number"
                placeholder="20"
                :rules="[() => !!Place.placename || '콘센트 개수을 입력해주세요']"
                required
                ></v-text-field>
            </v-list-item-title>
          </v-list-item>
        ...
        <v-list-item>
          <v-list-item>
            <v-icon color="light-green darken-2">mdi-information-outline</v-icon>
            <v-list-item-title>매장 설명</v-list-item-title>
            <v-list-item-title>
              <v-textarea
                filled
                :rules="[() => !!Place.placename || '매장 설명을 입력해주세요']"
                required
                color="light-green darken-2"
                v-model="Place.cafe.subdetail"
                placeholder="기타 세부 정보를 입력해주세요(여러줄 입력 가능)"
              ></v-textarea>
            </v-list-item-title>
          </v-list-item>
        </v-list-item>
      ...
      <v-divider class='mx-2'></v-divider>
        <h5 class='d-flex justify-start px-4 pt-4'>Menu</h5>
        <p class="d-flex justify-start px-8 font-weight-light caption grey--text lighten-4--text">
          <v-icon class="font-weight-light body-2 amber--text lighten-2--text mr-2">mdi-book-open-page-variant</v-icon>
          메뉴를 등록해주세요.
        </p>
      <v-divider class='mx-2'></v-divider>
        ...
        <v-list two-line>
          <div id="menu">
            <v-list-item v-for="(me,index) in Place.menu" :key="index" class='d-flex-center'>
              <v-list-item id="menu">
                <v-icon class='d-flex align-self-center' color="light-green darken-2">mdi-clipboard-plus-outline</v-icon>
                <v-list-item-title class='mx-2'>
                  <input type="text" class="form-control text-center" style="max" placeholder="메뉴명" v-model="me.name">
                </v-list-item-title>
                <v-list-item-title class='mx-2'>
                  <input type="text" class="form-control text-center" style="max" placeholder="가격" v-model="me.price">
            ...
            <div class="col-md-12 text-center mt-4">
              <button type="button" class="btn btn-sm btn-outline-secondary mr-3" @click="menuplus">
                메뉴 추가
              </button>
            </div>
          </div>
...
export default {
    ...
    methods:{        
        menuplus(){           
            this.Place.menu.push({
                name:"",
                price:'',
                menupicture:{name:"",data:""},                
            });

        },
        menuremove(){
            if(this.Place.cafe.menu.length >0)
            {
                this.$delete(this.Place.menu,this.Place.menu.length-1);
            }            
        },
        preprocess(){
            ...
            if( this.Place.cafe.alcoholonoff =="X")
                this.Place.cafe.alcoholonoff="False";
            else
                this.Place.cafe.alcoholonoff="True";
            if( this.Place.cafe.dessertonoff =="X")
                this.Place.cafe.dessertonoff="False";
            else
                this.Place.cafe.dessertonoff="True";
            if( this.Place.cafe.smokingroom =="X")
                this.Place.cafe.smokingroom="False";
            else
                this.Place.cafe.smokingroom="True";
        },
        registdetail(){
            // window.console.log("placepicture")
            // window.console.log(this.Place.placepicture)
            this.preprocess();            
            console.log(this.Place)
            axios_common            
            .post('/registdetail/1/',this.Place                        
            ).then(response=>{
